...
use backend\widgets\AdminGallery\GalleryDefaultImageAsset;
use Yii;
...
class GalleryBehavior extends Behavior {
    ...
    *                },
     ...
     *
     * @return \backend\widgets\AdminGallery\ImageQuery
     */
    ...
    public function deleteImages(array $ids) {
        $images = $this->owner->getGalleryImages()->all();
     ...
     * @return boolean
     */
    public function updateAlt(array $rows)
    {
        $images = $this->owner->galleryImages;
        $images = ArrayHelper::index($images, 'id');
        ...
        foreach ($rows as $row) {
            ...
            $images[$row['imageId']]->alt = $row['alt'];
            if (!$images[$row['imageId']]->save()) {
                $success = false;
            }
    ...
    {
        $success = true;
        $oldImages = $this->owner->getGalleryImages()->all();
        if ($oldBasePath === null) {
        ...
        // deleting old images
        if (!$this->owner->deleteImages(ArrayHelper::getColumn($oldImages, 'id'))) {
            $success = false;
        }
        return $success;
    }
