jQuery(function () {
     ...
     * Sends new order request
     */
    jQuery(document).on("blur", ".image-alt-input", function (event, ui) {

        var messages = jQuery(this).data("messages");

        var url = jQuery(".ajax-gallery-list").data("action-url");
        url = url + "?action=alt&modelId=" + jQuery(".ajax-gallery-list").data("model-id");
