...
use backend\widgets\AdminGallery\validators\GalleryAdditionalDataValidator;
use Yii;
...
class GalleryAjaxAction extends Action
{
    ...
    public function run($action, $modelId)
    {
        ...
        switch ($action) {
            ...
            case self::ACTION_UPDATE_ALT:
                return ['success' => $this->alt($model)];
     ...
     * @return boolean
     */
    protected function alt($model)
    {
        $data = Yii::$app->request->post($this->dataParameter);
        return $model->updateAlt($data);
    }
